...
/**
 * 💰8% DAI(美元稳定币)的静态奖励每60分钟重新分配给MoonWilly代币持有者。至少需要10,000个MoonWilly代币才能获得奖励。为什么选择DAI?持有者不需要担心监管机构冻结他们的资产或被列入黑名单，因为DAI由智能合约控制。对于USDC、USDT和BUSD则不是这样。

💧3%流动费用(发回LP)

🗣3%营销、发展、慈善(MoonWilly Treasury)

☯️每笔交易1%烧录到烧录钱包地址(黑洞)。刻录钱包地址被列入黑名单，无法获得DAI(美元稳定币)奖励。

📈📉销售时额外征收3%的税，以防止泵和转储(1%用于营销，1%用于BURN, 1%用于LP)

🗑Anti-dump特性:每次最多可售出总供应量的0.1%
...
燃烧储备330,000,000 -全部燃烧!

流动性5亿-锁定!
 * 
 */
///这个作为参考
contract cankao01 is ERC20, Ownable {
    using SafeMath for uint256;
